...
# 安裝 Rust 工具鏈（如果尚未安裝）
# 請參考：https://www.rust-lang.org/tools/install
```
...
應用程式會在開發模式下啟動，支援熱重載（Hot Reload）。
...
- **執行檔**：`src-tauri/target/release/theranotes.exe`
...
歡迎對 TheraNotes 做出貢獻！無論是回報問題、建議功能或提交程式碼都非常歡迎。
...
感謝所有使用 TheraNotes 的心理諮商師和實習生們。你們的回饋讓這個專案持續進步。
